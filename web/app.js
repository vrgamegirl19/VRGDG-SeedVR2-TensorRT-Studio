...
const selectWorkspaceOutput = (output) => { selectedWorkspaceOutput = output; outputPickerLabel.textContent = output.name; outputPickerThumb.autoplay = true; outputPickerThumb.muted = true; outputPickerThumb.playsInline = true; outputPickerThumb.src = output.url; outputPickerThumb.load(); outputPickerToggle.classList.add('has-selection'); closeOutputPicker(); };
...
async function loadOutputs() {
  try {
    const response = await fetch('/api/outputs'); const outputs = await response.json(); outputPickerMenu.innerHTML = ''; selectedWorkspaceOutput = null; outputPickerLabel.textContent = outputs.length ? 'Select a workspace output…' : 'No workspace outputs yet'; outputPickerThumb.removeAttribute('src'); outputPickerThumb.load(); outputPickerToggle.classList.remove('has-selection');
    outputs.forEach((output) => { const item = document.createElement('button'); item.type = 'button'; item.className = 'output-picker-item'; item.setAttribute('role', 'option'); const thumb = document.createElement('video'); thumb.className = 'output-item-thumb'; thumb.setAttribute('aria-hidden', 'true'); thumb.muted = true; thumb.autoplay = true; thumb.playsInline = true; thumb.preload = 'auto'; thumb.src = output.url; thumb.addEventListener('loadeddata', () => { thumb.currentTime = Math.min(0.25, thumb.duration || 0); }); const copy = document.createElement('span'); copy.className = 'output-item-copy'; const name = document.createElement('strong'); name.textContent = output.name; const kind = document.createElement('small'); kind.textContent = output.reprocessable ? 'TensorRT result' : 'Workspace result'; copy.append(name, kind); item.append(thumb, copy); item.addEventListener('click', () => selectWorkspaceOutput(output)); outputPickerMenu.append(item); });
...
}$('#load-output').addEventListener('click', () => { const output = selectedWorkspaceOutput; if (!output) return; currentOutputPath = output.path || output.url; currentOutputReprocessable = Boolean(output.reprocessable); fps = Number(output.fps) || 30; $('#reprocess-post').hidden = !currentOutputReprocessable; $('#open-output-folder').hidden = false; loadVideo(after, output.url); $('#media-info').textContent = currentOutputReprocessable ? 'Previous result loaded · post-only reprocess available' : 'Previous workspace result loaded'; });
...
const beginNewProject = (file) => { projectToken += 1; const previousJob = activeJob; activeJob = null; lastFailedJob = null; currentOutputPath = ''; currentOutputReprocessable = false; stopElapsed(0); setRenderBusy(false); loadVideo(after, ''); $('#reprocess-post').hidden = true; $('#open-output-folder').hidden = true; $('#media-info').textContent = `${file.name} · new project`; $('#render-status').textContent = previousJob ? 'New project ready · previous render continues in background' : 'Ready'; };
