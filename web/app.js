...
const selectWorkspaceOutput = (output) => { selectedWorkspaceOutput = output; outputPickerLabel.textContent = output.name; outputPickerThumb.src = output.url; outputPickerThumb.load(); outputPickerToggle.classList.add('has-selection'); closeOutputPicker(); };
...
async function loadOutputs() {
  try {
    ...
    outputs.forEach((output) => { const item = document.createElement('button'); item.type = 'button'; item.className = 'output-picker-item'; item.setAttribute('role', 'option'); const thumb = document.createElement('span'); thumb.className = 'output-item-thumb'; thumb.muted = true; thumb.preload = 'metadata'; thumb.src = output.url; thumb.addEventListener('loadedmetadata', () => { thumb.currentTime = Math.min(0.1, thumb.duration || 0); }); const copy = document.createElement('span'); copy.className = 'output-item-copy'; const name = document.createElement('strong'); name.textContent = output.name; const kind = document.createElement('small'); kind.textContent = output.reprocessable ? 'TensorRT result' : 'Workspace result'; copy.append(name, kind); item.append(thumb, copy); item.addEventListener('click', () => selectWorkspaceOutput(output)); outputPickerMenu.append(item); });
