...
const selectWorkspaceOutput = (output) => { selectedWorkspaceOutput = output; outputPickerLabel.textContent = output.name; outputPickerThumb.style.backgroundImage = 'url(' + output.url + ')'; outputPickerToggle.classList.add('has-selection'); closeOutputPicker(); };
...
async function loadOutputs() {
  try {
    const response = await fetch('/api/outputs'); const outputs = await response.json(); outputPickerMenu.innerHTML = ''; selectedWorkspaceOutput = null; outputPickerLabel.textContent = outputs.length ? 'Select a workspace output…' : 'No workspace outputs yet'; outputPickerThumb.style.backgroundImage = 'none'; outputPickerToggle.classList.remove('has-selection');
    outputs.forEach((output) => { const item = document.createElement('button'); item.type = 'button'; item.className = 'output-picker-item'; item.setAttribute('role', 'option'); const thumb = document.createElement('span'); thumb.className = 'output-item-thumb'; thumb.style.backgroundImage = 'url(' + output.url + ')'; const copy = document.createElement('span'); copy.className = 'output-item-copy'; const name = document.createElement('strong'); name.textContent = output.name; const kind = document.createElement('small'); kind.textContent = output.reprocessable ? 'TensorRT result' : 'Workspace result'; copy.append(name, kind); item.append(thumb, copy); item.addEventListener('click', () => selectWorkspaceOutput(output)); outputPickerMenu.append(item); });
